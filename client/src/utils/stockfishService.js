// Simpler implementation that doesn't rely on npm package
class StockfishService {
  ...
  init() {
    try {
      ...
      this.worker = new Worker('https://cdn.jsdelivr.net/npm/stockfish@11/src/stockfish.js');
  ...
  getNextMove(callback) {
    ...
    this.onMessage = (event) => {
      ...
      if (event.includes('bestmove')) {
        const match = event.match(/bestmove\s+(\w+)/);
        if (match && match[1]) {
